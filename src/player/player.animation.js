function playAnimation(iso) {
  ...
  if (animationName.includes("climbing")) {
    if (player.iso.body.velocity.z !== 0) {
      iso.animations.play(animationName)
    } 
    else if (player.iso.body.velocity.x !== 0) {
      iso.animations.play("climbing-x")
    }
    else if (player.iso.body.velocity.y !== 0) {
      iso.animations.play("climbing-y")
    }
    else {
      iso.animations.stop(null, false)
    }
  } else if (iso.animations.currentAnim.name !== animationName) {
    iso.animations.play(animationName)
  }
}
